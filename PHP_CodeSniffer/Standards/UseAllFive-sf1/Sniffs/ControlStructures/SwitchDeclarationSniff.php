<?php
/**
 * UseAllFiveSf1_Sniffs_ControlStructures_SwitchDeclarationSniff.
 *
 * PHP version 5
 *
 * @category  PHP
 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class UseAllFiveSf1_Sniffs_ControlStructures_SwitchDeclarationSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        $nextCase      = $stackPtr;
        $indent        = 2;
        $caseAlignment = ($switch['column'] + $indent);
        $caseCount     = 0;
        $foundDefault  = false;
        ...
        while (($nextCase = $this->_findNextCase($phpcsFile, ($nextCase + 1), $switch['scope_closer'])) !== false) {
            ...
            if ($tokens[$nextCase]['column'] !== $caseAlignment) {
                $error = strtoupper($type).' keyword must be indented '. $indent .' spaces from SWITCH keyword';
            ...
            if ($tokens[$nextCloser]['scope_condition'] === $nextCase) {
                ...
                // the default case.
                if ($tokens[$nextCloser]['column'] !== ($caseAlignment + $indent)) {
